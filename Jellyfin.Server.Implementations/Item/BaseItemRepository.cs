...
#pragma warning disable CA1307 // Specify StringComparison for clarity
        ...
        InternalItemsQuery filter)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filter.HasNoAudioTrackWithLanguage))
        {
            ...
            if (lang is not null)
            {
                var langSet = new HashSet<string?>(lang.ThreeLetterISOLanguageNames);
                baseQuery = baseQuery
                    .Where(e => !e.MediaStreams!.Any(f => f.StreamType == MediaStreamTypeEntity.Audio && langSet.Contains(f.Language)));
            }
        }
        ...
        if (!string.IsNullOrWhiteSpace(filter.HasNoInternalSubtitleTrackWithLanguage))
        {
            ...
            if (lang is not null)
            {
                var langSet = new HashSet<string?>(lang.ThreeLetterISOLanguageNames);
                baseQuery = baseQuery
                    .Where(e => !e.MediaStreams!.Any(f => f.StreamType == MediaStreamTypeEntity.Subtitle && !f.IsExternal && langSet.Contains(f.Language)));
            }
        }
        ...
        if (!string.IsNullOrWhiteSpace(filter.HasNoSubtitleTrackWithLanguage))
        {
            ...
            if (lang is not null)
            {
                var langSet = new HashSet<string?>(lang.ThreeLetterISOLanguageNames);
                baseQuery = baseQuery
                    .Where(e => !e.MediaStreams!.Any(f => f.StreamType == MediaStreamTypeEntity.Subtitle && langSet.Contains(f.Language)));
            }
        }
